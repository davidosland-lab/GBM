...
def test_normalize_text_applies_scope_v2_aliases() -> None:
    assert normalize_text("Voranigo") == "vorasidenib"
    assert normalize_text("AG-881") == "vorasidenib"
    assert normalize_text("FUS") == "focused ultrasound"
    assert normalize_text("BBBO") == "blood-brain barrier opening"
    assert normalize_text("PLX3397") == "pexidartinib"
    assert normalize_text("MES-like") == "MES-like"
